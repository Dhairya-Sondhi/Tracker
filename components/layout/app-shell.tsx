...
export function AppShell({children,accountSource="self"}:{children:React.ReactNode;accountSource?:"self"|"children"}){
 const pathname=usePathname(),router=useRouter(),[account,setAccount]=useState<Account|null>(null),[menu,setMenu]=useState(false),[navigatingTo,setNavigatingTo]=useState<string|null>(null),accountResolved=useRef(false);
 ...
 useEffect(()=>{router.prefetch("/today");router.prefetch("/dashboard")},[router]);
 ...
 return <div data-app-theme="workspace" className="min-h-screen w-full overflow-x-clip bg-canvas text-ink selection:bg-violet/40">
  {navigatingTo&&<div role="status" aria-label={`Opening ${navigatingTo.slice(1)}`} className="fixed inset-x-0 top-0 z-[10000] h-0.5 overflow-hidden bg-lavender/15"><span className="block h-full w-1/2 animate-pulse rounded-full bg-lavender"/></div>}
  ...
  <aside className="fixed inset-y-0 left-0 z-50 hidden w-[88px] flex-col items-center border-r border-line bg-surface/80 py-5 backdrop-blur-2xl lg:flex">
   ...
   <nav className="mt-12 flex w-full flex-col items-center gap-3" aria-label="App navigation">{nav.map(({label,href,icon:Icon})=>{const active=pathname===href||(href==="/dashboard"&&pathname==="/analytics");return <Link key={href} href={href} prefetch onClick={()=>{if(!active)setNavigatingTo(href)}} aria-current={active?"page":undefined} className={cn("focus-ring group flex w-[68px] flex-col items-center gap-1.5 rounded-2xl py-3 text-[9px] font-semibold transition-all",active?"border border-line bg-surface-raised text-ink shadow-soft":"text-muted hover:bg-surface hover:text-ink",navigatingTo===href&&"bg-surface text-ink")}><Icon aria-hidden="true" size={18} className={active?"text-lavender":""}/>{label}</Link>})}</nav>
  ...
  <nav className="fixed inset-x-3 bottom-[calc(.75rem+env(safe-area-inset-bottom))] z-50 grid min-w-0 grid-cols-[repeat(3,minmax(0,1fr))] overflow-hidden rounded-[1.35rem] border border-line bg-surface-raised/90 p-1.5 shadow-[0_18px_55px_rgba(0,0,0,.45)] backdrop-blur-2xl lg:hidden" aria-label="Mobile app navigation">
   {mobileNav.map(({label,href,icon:Icon})=>{const active=pathname===href||(href==="/dashboard"&&pathname==="/analytics");return <Link key={href} href={href} prefetch onClick={()=>{if(!active)setNavigatingTo(href)}} aria-current={active?"page":undefined} className={cn("focus-ring flex min-h-13 min-w-0 items-center justify-center gap-1.5 rounded-2xl px-1 text-[10px] font-semibold transition-all",active?"border border-line bg-surface-raised text-ink shadow-soft":"text-muted hover:bg-surface",navigatingTo===href&&"bg-surface text-ink")}><Icon aria-hidden="true" size={16} className="shrink-0"/><span className="hidden min-w-0 truncate min-[360px]:inline">{label}</span></Link>})}
  </nav>
 </div>
}
